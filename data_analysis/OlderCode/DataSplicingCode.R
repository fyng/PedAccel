...
gt3xfile <-
  system.file(
    "extdata", "Patient9_Data.gt3x",
    package = "read.gt3x")
gt3xfolders <- unzip.gt3x("Patient9_Data.gt3x", location = tempdir())
gt3xfolder <- gt3xfolders[1]
X <- read.gt3x(gt3xfolder)
X <- as.data.frame(X)

# Splice data between two date/times - *times are in GMT*
start_datetime <- as.POSIXct("2024-01-16 13:53:00")
end_datetime <- as.POSIXct("2024-01-16 15:13:00")

# Subset data - *change file path for your system*
subset_df <- subset(X, time >= start_datetime & time <= end_datetime)
file_path <- "C:/Users/sidha/PycharmProjects/PedAccel/Patient9_SBSChange4.csv"
