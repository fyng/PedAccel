...
def FFTPlot(y, sr=100, lower_freq=0, upper_freq=20, lower_cutoff=40, upper_cutoff=10000, title='FFT Plot', figsize=(12, 6), xlim=20, ylim=30):
    """
...
# def FFTPlot(FFTData, freq,sr=100,lower_freq=0, upper_freq=20, lower_cutoff=40, upper_cutoff=10000, standardize = False, title='FFT Plot'):
#     """
...
#     print(f'The maximum frequency is {(max( (v, i) for i, v in enumerate(new_array[lower_freq_index:upper_freq_index]) )[1])/T}')
#     #Standardize such that 10 is maximum amplitude for each plot
#     if(standardize):
#         maximum = max(new_array)
#         new_array = new_array*(10/maximum)
#     plt.plot(f_oneside[lower_freq_index:upper_freq_index], new_array[lower_freq_index:upper_freq_index])
#     plt.xlabel('Freq (Hz)')
#     plt.ylabel('FFT Amplitude |X(freq)|')
#     plt.title(title)
