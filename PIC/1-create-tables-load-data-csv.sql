...
CREATE TABLE CHARTEVENTS (	-- rows=1048575
   ROW_ID INT NOT NULL,
   SUBJECT_ID INTEGER NOT NULL,
   HADM_ID INTEGER NOT NULL,
  ...
  CONSTRAINT CHARTEVENTS_ROW_ID UNIQUE (ROW_ID)	-- nvals=1048575
  );

\COPY chartevents FROM 'CHARTEVENTS.csv' CSV HEADER NULL ''

DROP TABLE IF EXISTS DIAGNOSES_ICD;
CREATE TABLE DIAGNOSES_ICD (	-- rows=22712
   ROW_ID INTEGER NOT NULL,
   SUBJECT_ID INTEGER NOT NULL,
   HADM_ID INTEGER NOT NULL,
   SEQ_NUM SMALLINT NOT NULL,
   ICD10_CODE_CN VARCHAR(255) NOT NULL,	
   DIAG_CATEGORY VARCHAR(255) NOT NULL,
  CONSTRAINT DIAGNOSES_ICD_ROW_ID UNIQUE (ROW_ID)	-- nvals=22712
  );

\COPY DIAGNOSES_ICD FROM 'DIAGNOSES_ICD.csv' CSV HEADER NULL ''

DROP TABLE IF EXISTS D_ICD_DIAGNOSES;
CREATE TABLE D_ICD_DIAGNOSES (	-- rows=25379
   ROW_ID SMALLINT NOT NULL,
   ICD10_CODE_CN VARCHAR(255) NOT NULL,
   ICD10_CODE CHAR(7),	
   TITLE_CN VARCHAR(255) NOT NULL,	
   TITLE VARCHAR(255) NOT NULL,	
  CONSTRAINT D_ICD_DIAGNOSES_ROW_ID UNIQUE (ROW_ID),	-- nvals=25379
  CONSTRAINT D_ICD_DIAGNOSES_ICD10_CODE_CN UNIQUE (ICD10_CODE_CN)	-- nvals=25379
  );

\COPY D_ICD_DIAGNOSES FROM 'D_ICD_DIAGNOSES.csv' CSV HEADER NULL ''

DROP TABLE IF EXISTS D_ITEMS;
CREATE TABLE D_ITEMS (	-- rows=479
   ROW_ID SMALLINT NOT NULL,
   ITEMID VARCHAR(255) NOT NULL,
...
CREATE TABLE ICUSTAYS (	-- rows=13941
   ...
   SUBJECT_ID INTEGER NOT NULL,
   HADM_ID INTEGER NOT NULL,
   ICUSTAY_ID VARCHAR(10) NOT NULL,
...
CREATE TABLE LABEVENTS (	-- rows=1048575
   ROW_ID INT NOT NULL,
   SUBJECT_ID INTEGER NOT NULL,
   HADM_ID INTEGER NOT NULL,
   ITEMID SMALLINT NOT NULL,
   ...
   VALUEUOM VARCHAR(255),	
   FLAG VARCHAR(255),	
  CONSTRAINT LABEVENTS_ROW_ID UNIQUE (ROW_ID)	-- nvals=1048575
  );
...
CREATE TABLE PRESCRIPTIONS (	-- rows=1048575
   ROW_ID INTEGER NOT NULL,
   SUBJECT_ID INTEGER NOT NULL,
   HADM_ID INTEGER NOT NULL,
   ICUSTAY_ID VARCHAR(10),
  ...
  CONSTRAINT PRESCRIPTIONS_ROW_ID UNIQUE (ROW_ID)	-- nvals=1048575
  );
