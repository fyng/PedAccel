...
all_win_size_dict = {'x_data':[], 'sbs_data':[], 'win_size_data':[], 'label_data':[], 'one_sided':[], 'patient_id':[]}
...
for p in patients:
    ...
    if np.any(np.abs(patient_multi['SBS']) >= 2):
        ...
        win_size_dict = {'x_data':[], 'sbs_data':[], 'win_size_data':[], 'label_data':[], 'one_sided':[], 'patient_id':[]}
...
for a, b in zip(win_size_a, win_size_b):
    ...
    for i, ax in enumerate(axs.flatten()):  
        conditions = (win_size['label_data']==labels[i]) & (win_size['win_size_data'] == abs(a))
        sns.boxplot(legend = False, data=win_size[conditions], \
            x='sbs_data', y='x_data', hue='one_sided', ax=ax, showfliers=False)
        ax.set_xlabel('SBS')
        ax.margins(0.1, 0.1)
    ...
    fn = "images_boxplot_std/win" + str(abs(a)) + "_" + str(j) + ".png"
...
fn = "images_boxplot_std/heat_" + str(j) + ".png"
fig.tight_layout()  # otherwise the right y-label is slightly clipped
fig.savefig(fn)
plt.close(fig)
