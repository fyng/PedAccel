...
for file in os.listdir(ehr_dir):
    if file.endswith('.csv.gz'):
        ...
        f.write(file + '\t' + str(data.shape[0]) + '\n')
        for feature in data.columns.values:
            f.write(feature + '\t')
            f.write(str(data[feature].isna().sum()/len(data[feature])) + '\n')
...
all_win_size_dict = {'age_m':[], 'x_data':[], 'sbs_data':[], 'win_size_data':[], 'label_data':[], \
    'one_sided':[], 'patient_id':[], 'mean_data':[]}
        ...
        for a, b in zip(win_size_a, win_size_b):
            ...
            for l in labels:
                ...
                for start, end in zip(starts, ends):
                    ...
                    win_size_dict['x_data'].append(np.std(x_t))
                    win_size_dict['mean_data'].append(np.mean(x_t))
...
for a, b in zip(win_size_a, win_size_b):
    ...
    for i, ax in enumerate(axs.flatten()):  
        ...
        sns.boxplot(legend = True, data=win_size[conditions], \
            x='sbs_data', y='x_data', hue='age_bin', ax=ax, showfliers=False)
        ax.set_xlabel('SBS')
        ax.margins(0.1, 0.1)
        ...
        for m, age in enumerate(lab):
            ...
            x_ = win_size[conditions]['mean_data'].to_numpy()
    ...
    fn = folder +'win'+ str(abs(a)) + "_" +str(abs(b)) + "_"+ str(j) + ".png"
