...
for file in os.listdir(ehr_dir):
    if file.endswith('.csv.gz'):
        ...
        f.write(file)
        f.write(np.array2string(data.columns.values))
...
all_win_size_dict = {'age_m':[], 'x_data':[], 'sbs_data':[], 'win_size_data':[], 'label_data':[], 'one_sided':[], 'patient_id':[]}
...
for p in patients:
    ...
    if np.any(np.abs(patient_multi['SBS']) >= 2):
        ...
        win_size_dict = {'x_data':[], 'sbs_data':[], 'win_size_data':[], 'label_data':[], 'one_sided':[], 'patient_id':[], 'age_m':[]}
...
for a, b in zip(win_size_a, win_size_b):
    ...
    fn = "images_age_box_plot/win" + str(abs(a)) + "_" +str(abs(b)) + "_"+ str(j) + ".png"
...
fn = "images_age_box_plot/heat_" + str(j) + ".png"
fig.tight_layout()  # otherwise the right y-label is slightly clipped
fig.savefig(fn)
plt.close(fig)
