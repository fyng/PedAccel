...
wlen = 5
freqData, freqBins = Frequency_Domain.freq_domain_rollingAvg(signal,wlen)
itx = Frequency_Domain.inverse_FFT(freqData)
time = list(myData['Time(s)'])
plt.scatter(time[:-wlen+1], itx)
plt.title('IFFT after rolling avg in freq domain')
plt.show()
