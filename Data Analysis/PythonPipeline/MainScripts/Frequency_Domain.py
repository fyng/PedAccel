...
def my_comb_filter(signal,f0=1,Q=10,fs=100):
    for i in range(51):
        b, a = scipy.signal.iirnotch(i*f0, Q, fs)
        signal = scipy.signal.lfilter(b, a, signal)
    freqData, freqBins = FFTData(signal,100)
    #Plot FFT Data
    FFTPlot(freqData,freqBins, 100, lower_freq=.3, upper_freq=20, lower_cutoff=.3, upper_cutoff=100000,title='FFT my Comb Filter')
